...
export default function ResultsPage() {
  ...
  const fetchResults = async () => {
    setLoading(true)
    const url = sessionId ? `/api/results?sessionId=${sessionId}` : '/api/results'
    ...
    <AdminLayout>
      <div className="p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-900">Hasil & Statistik</h1>
          <a href={`/api/results/export${sessionId ? `?sessionId=${sessionId}` : ''}`}
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan={7} className="px-6 py-8 text-center text-gray-500">Belum ada hasil</td></tr>
              ) : results.map(r => (
                <tr key={r.id}>
